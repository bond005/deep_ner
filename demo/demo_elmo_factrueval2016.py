...
def train(factrueval2016_devset_dir: str, split_by_paragraphs: bool, elmo_will_be_tuned: bool,
    ...
    if os.path.isfile(model_name):
        ...
        if collection3_dir is not None:
            X_train_, y_train_ = load_dataset_from_brat(collection3_dir,
            ...
            print('')
            if (n_max_samples == 0) or (
                    (n_max_samples > 0) and (X_train.shape[0] < n_max_samples)):
                if n_max_samples > 0:
                    index = sample_from_dataset(y=y_train,
                                                n=n_max_samples - X_train.shape[0])
...
def main():
    ...
    parser.add_argument('--finetune_elmo', dest='finetune_elmo', required=False, action='store_true',
                        default=False, help='Will be the ELMo and last layers finetuned together? Or the ELMo will be frozen?')
