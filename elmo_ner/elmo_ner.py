...
class ELMo_NER(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X: Union[list, tuple, np.array], y: Union[list, tuple, np.array]):
        ...
        if self.verbose:
            if X_val is None:
                ...
                elmo_ner_logger.info('Epoch   Train acc.   Test acc.')
        ...
        try:
            ...
            for epoch in range(self.max_epochs):
                ...
                if bounds_of_batches_for_validation is not None:
                    ...
                    if self.verbose:
                        elmo_ner_logger.info('{0:>5}   {1:>10.8f}   {2:>10.8f}'.format(epoch, acc_train, acc_test))
                else:
                    if best_acc is None:
                        best_acc = acc_train
                        saver.save(self.sess_, tmp_model_name)
                        ...
                        n_epochs_without_improving += 1
                    if self.verbose:
                        elmo_ner_logger.info('{0:>5}   {1:>10.8f}'.format(epoch, acc_train))
    ...
    def is_fitted(self):
        check_is_fitted(self, ['classes_list_', 'logits_', 'transition_params_', 'tokenizer_', 'input_tokens_',
                               'sequence_lengths_', 'y_ph_', 'sess_'])
    ...
    def fill_feed_dict(self, X: List[np.array], y: np.array=None) -> dict:
        ...
        feed_dict = {ph: x for ph, x in zip([self.input_tokens_, self.sequence_lengths_], X)}
        if y is not None:
    ...
    def tokenize_all(self, X: Union[list, tuple, np.array], y: Union[list, tuple, np.array]=None) -> \
            Tuple[List[np.ndarray], Union[np.ndarray, None]]:
        tokens = []
        lenghts = []
        ...
        return [np.array(tokens, dtype=np.str), np.array(lenghts, dtype=np.int32)], \
               (None if y is None else np.array(y_tokenized))
    ...
    def load_all(self, new_params: dict):
        ...
        if hasattr(self, 'sess_'):
            ...
            del self.sess_
        tf.reset_default_graph()
        is_fitted = ('classes_list_' in new_params) and ('model_name_' in new_params)
        model_files = list(
            filter(
